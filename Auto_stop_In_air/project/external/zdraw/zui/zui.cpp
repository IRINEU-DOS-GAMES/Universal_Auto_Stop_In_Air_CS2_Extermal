...
namespace
{
	...
	constexpr int keybind_mode_always_on{ static_cast< int >( zui::keybind_mode::always_on ) };
	constexpr int keybind_mode_toggle{ static_cast< int >( zui::keybind_mode::toggle ) };
	constexpr int keybind_mode_hold{ static_cast< int >( zui::keybind_mode::hold ) };

	[[nodiscard]] int normalize_keybind_mode( int mode ) noexcept
	{
		switch ( mode )
		{
		case keybind_mode_always_on:
		case keybind_mode_toggle:
		case keybind_mode_hold:
			return mode;
		default:
			return keybind_mode_hold;
		}
	}
	...
	public:
		...
		void close( widget_id id )
		{
			for ( auto& o : this->m_overlays )
			{
				if ( o && o->id( ) == id )
				{
					// Close immediately so a second click on the same widget
					// reliably toggles popup visibility in the same frame.
					o.reset( );
					break;
				}
			}
	...
	public:
		...
		bool process_input( const input_state& input ) override
		{
			...
			const auto content_h = this->get_content_height( );
			const auto max_scroll = std::max( 0.0f, content_h - dropdown.h );

			if ( dropdown.contains( input.mouse_x, input.mouse_y ) && input.scroll_delta != 0.0f )
			{
				this->m_scroll_offset -= input.scroll_delta * 30.0f;
				this->m_scroll_offset = std::clamp( this->m_scroll_offset, 0.0f, max_scroll );
			}
			...
			if ( input.mouse_clicked && dropdown.contains( input.mouse_x, input.mouse_y ) )
			{
				...
				const auto item_height = this->get_item_height( );

				for ( int i = 0; i < static_cast< int >( this->m_items.size( ) ); ++i )
				{
					const auto item_y = dropdown.y + padding_top + i * item_height - this->m_scroll_offset;
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			for ( int i = 0; i < static_cast< int >( this->m_items.size( ) ); ++i )
			{
				...
				if ( item_rect.bottom( ) < dropdown.y || item_rect.y > dropdown.bottom( ) )
				{
					continue;
				}

				const auto item_delay = std::min( i * 0.04f, 0.6f );
				const auto item_progress = std::clamp(
					( this->m_open_anim - item_delay ) / std::max( 0.001f, 1.0f - item_delay ),
					0.0f,
					1.0f );
				...
				const auto visual_pad_y = 1.0f;
				const auto visual_y = item_rect.y + slide_offset + visual_pad_y;
				const auto visual_h = std::max( 1.0f, item_rect.h - visual_pad_y * 2.0f );

				if ( selected_ease > 0.01f )
				{
					auto border_left = style.accent;
					border_left.a = static_cast< std::uint8_t >( 220 * item_alpha * selected_ease );
					zdraw::rect_filled( item_rect.x, visual_y, 2.5f, visual_h, border_left );
			...
			if ( max_scroll > 0.5f )
			{
				const auto track_w = 3.0f;
				const auto track_x = dropdown.right( ) - track_w - 2.0f;
				auto track_col = style.combo_popup_border;
				track_col.a = static_cast< std::uint8_t >( track_col.a * alpha_mult * 0.7f );
				zdraw::rect_filled( track_x, dropdown.y + 2.0f, track_w, std::max( 1.0f, animated_h - 4.0f ), track_col );

				const auto visible_ratio = std::clamp( dropdown.h / content_h, 0.05f, 1.0f );
				const auto thumb_h = std::max( 16.0f, dropdown.h * visible_ratio );
				const auto thumb_range = std::max( 1.0f, dropdown.h - thumb_h );
				const auto thumb_t = this->m_scroll_offset / max_scroll;
				const auto thumb_y = dropdown.y + thumb_range * thumb_t;
				auto thumb_col = style.accent;
				thumb_col.a = static_cast< std::uint8_t >( 200.0f * alpha_mult );
				zdraw::rect_filled( track_x, thumb_y, track_w, thumb_h, thumb_col );
			}
		}
	...
	private:
		[[nodiscard]] float get_item_height( ) const
		{
			return 20.0f;
		}

		[[nodiscard]] float get_content_height( ) const
		{
			const auto padding_top{ 6.0f };
			const auto padding_bottom{ 6.0f };
			return static_cast< float >( this->m_items.size( ) ) * this->get_item_height( ) + padding_top + padding_bottom;
		}
		...
		[[nodiscard]] zui::rect get_dropdown_rect( ) const
		{
			...
			const auto content_h = this->get_content_height( );
			const auto max_visible_h = this->get_item_height( ) * 12.0f + 12.0f;
			const auto desired_h = std::min( content_h, max_visible_h );
			constexpr auto margin{ 8.0f };
			const auto down_y = anchor.bottom( ) + 4.0f;
			const auto space_below = std::max( 60.0f, display_h - margin - down_y );
			const auto space_above = std::max( 60.0f, anchor.y - margin - 4.0f );
			const auto open_up = space_below < desired_h && space_above > space_below;
			const auto dropdown_h = std::max( 60.0f, std::min( desired_h, open_up ? space_above : space_below ) );
			const auto dropdown_y = open_up ? ( anchor.y - 4.0f - dropdown_h ) : down_y;

			return zui::rect
			{
				std::clamp( anchor.x, margin, std::max( margin, display_w - this->m_width - margin ) ),
				dropdown_y,
				this->m_width,
				dropdown_h
			};
	...
	{
	public:
		multicombo_overlay( widget_id id, const zui::rect& anchor, float width, const char* const* items, int items_count, std::uint8_t* selected_items, bool require_one_selected, std::function<void( )> on_change = nullptr ) : overlay{ id, anchor }, m_width{ width }, m_items_count{ items_count }, m_selected_items{ selected_items }, m_require_one_selected{ require_one_selected }, m_on_change{ std::move( on_change ) }
		{
			...
			if ( selected_items )
			{
				for ( int i = 0; i < items_count; ++i )
				{
					this->m_check_anims[ i ] = selected_items[ i ] != 0u ? 1.0f : 0.0f;
				}
			}
		}
			...
			if ( input.mouse_clicked && dropdown.contains( input.mouse_x, input.mouse_y ) )
			{
				...
				for ( int i = 0; i < this->m_items_count; ++i )
				{
					...
					if ( item_rect.contains( input.mouse_x, input.mouse_y ) )
					{
						...
						{
							const auto currently_selected = this->m_selected_items[ i ] != 0u;
							if ( currently_selected )
							{
								auto selected_count{ 0 };
								for ( int j = 0; j < this->m_items_count; ++j )
								{
									selected_count += this->m_selected_items[ j ] != 0u ? 1 : 0;
								}

								// Some widgets require at least one selected item (ex: hitboxes).
								if ( this->m_require_one_selected && selected_count <= 1 )
								{
									return true;
								}
							}

							this->m_selected_items[ i ] = currently_selected ? 0u : 1u;
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			for ( int i = 0; i < this->m_items_count; ++i )
			{
				...
				const auto is_selected = this->m_selected_items && this->m_selected_items[ i ] != 0u;
				const auto is_hovered = item_rect.contains( input.mouse_x, input.mouse_y ) && !this->m_closing;

				auto& selected_anim = this->m_check_anims[ i ];
				const auto selected_target = is_selected ? 1.0f : 0.0f;
				selected_anim += ( selected_target - selected_anim ) * std::min( 12.0f * dt, 1.0f );
				const auto selected_ease = zui::ease::out_quad( selected_anim );
				...
				auto text_col = style.text;
				const auto selected_text_col = zui::lerp( style.text, style.accent, 0.4f );
				text_col = zui::lerp( text_col, selected_text_col, selected_ease );
		...
		[[nodiscard]] const std::string& get_display_text( ) const
		{
			if ( this->m_display_text_dirty )
			{
				...
				for ( int i = 0; i < this->m_items_count; ++i )
				{
					if ( this->m_selected_items && this->m_selected_items[ i ] != 0u )
					{
						++selected_count;
				...
				if ( selected_count == this->m_items_count && this->m_items_count > 0 )
				{
					this->m_cached_display_text = "All";
				}
				else if ( this->m_cached_display_text.empty( ) )
				{
	...
	private:
		...
		[[nodiscard]] float get_content_height( ) const
		{
			const auto padding_top{ 6.0f };
			const auto padding_bottom{ 6.0f };
			return static_cast< float >( this->m_items_count ) * this->get_item_height( ) + padding_top + padding_bottom;
		}
		...
		std::vector<std::string> m_items{};
		int m_items_count{ 0 };
		std::uint8_t* m_selected_items{ nullptr };
		bool m_require_one_selected{ true };
		std::function<void( )> m_on_change{};
	...
	std::unordered_map<widget_id, int>& get_combo_values( )
	{
		static std::unordered_map<widget_id, int> values{};
		return values;
	}

	std::unordered_map<widget_id, bool>& get_combo_dirty_states( )
	{
		static std::unordered_map<widget_id, bool> dirty_states{};
		return dirty_states;
	}

	std::unordered_map<widget_id, std::vector<std::uint8_t>>& get_multicombo_values( )
	{
		static std::unordered_map<widget_id, std::vector<std::uint8_t>> values{};
		return values;
	}
	...
	enum class color_picker_mode : int
	{
		static_mode = 0,
		pulse = 1,
		rainbow = 2
	...
	constexpr float k_color_picker_rgb_speed_min{ 0.1f };
	constexpr float k_color_picker_rgb_speed_max{ 5.0f };

	[[nodiscard]] color_picker_mode clamp_color_picker_mode( int mode )
	{
		if ( mode < static_cast< int >( color_picker_mode::static_mode ) )
		{
			return color_picker_mode::static_mode;
		}

		if ( mode > static_cast< int >( color_picker_mode::rainbow ) )
		{
			return color_picker_mode::rainbow;
		}

		return static_cast<color_picker_mode>( mode );
	}

	std::unordered_map<widget_id, int>& get_color_picker_modes( )
	{
		static std::unordered_map<widget_id, int> modes{};
		return modes;
	}

	std::unordered_map<widget_id, zdraw::rgba>& get_color_picker_base_colors( )
	{
		static std::unordered_map<widget_id, zdraw::rgba> colors{};
		return colors;
	}

	std::unordered_map<widget_id, float>& get_color_picker_rainbow_speeds( )
	{
		static std::unordered_map<widget_id, float> speeds{};
		return speeds;
	}

	std::unordered_map<widget_id, float>& get_color_picker_rainbow_phase_offsets( )
	{
		static std::unordered_map<widget_id, float> phase_offsets{};
		return phase_offsets;
	}

	std::unordered_map<widget_id, zdraw::rgba*>& get_color_picker_targets( )
	{
		static std::unordered_map<widget_id, zdraw::rgba*> targets{};
		return targets;
	}

	[[nodiscard]] float get_color_picker_time_seconds( )
	{
		return static_cast< float >( ::GetTickCount64( ) ) * 0.001f;
	}

	[[nodiscard]] float evaluate_rainbow_hue_degrees( float rainbow_speed, float phase_offset )
	{
		const auto time_s = get_color_picker_time_seconds( );
		const auto speed = std::clamp( rainbow_speed, k_color_picker_rgb_speed_min, k_color_picker_rgb_speed_max );
		auto hue_deg = std::fmod( time_s * 120.0f * speed + phase_offset, 360.0f );
		if ( hue_deg < 0.0f )
		{
			hue_deg += 360.0f;
		}
	...
	[[nodiscard]] zdraw::rgba evaluate_color_picker_preview( color_picker_mode mode, const zdraw::rgba& base_color, float rainbow_speed, float rainbow_phase_offset = 0.0f )
	{
		const auto time_s = get_color_picker_time_seconds( );
		const auto speed = std::clamp( rainbow_speed, k_color_picker_rgb_speed_min, k_color_picker_rgb_speed_max );
		...
		switch ( mode )
		{
		case color_picker_mode::pulse:
		{
			const auto phase = std::sin( time_s * 3.0f ) * 0.5f + 0.5f;
			const auto factor = 0.45f + phase * 0.55f;

			return zdraw::rgba
			{
				static_cast< std::uint8_t >( std::clamp( base_color.r * factor, 0.0f, 255.0f ) ),
				static_cast< std::uint8_t >( std::clamp( base_color.g * factor, 0.0f, 255.0f ) ),
				static_cast< std::uint8_t >( std::clamp( base_color.b * factor, 0.0f, 255.0f ) ),
				base_color.a
			};
		}

		case color_picker_mode::rainbow:
		{
			const auto base_hsv = zui::rgb_to_hsv( base_color );
			const auto hue_deg = evaluate_rainbow_hue_degrees( speed, rainbow_phase_offset );
			auto rainbow_color = zui::hsv_to_rgb( hue_deg, base_hsv.s, base_hsv.v );
			rainbow_color.a = base_color.a;
			return rainbow_color;
		}

		case color_picker_mode::static_mode:
		default:
			return base_color;
		}
	}
	...
	public:
		...
		{
			auto& mode_value = this->mode_ref( );
			mode_value = static_cast< int >( clamp_color_picker_mode( mode_value ) );
			this->set_rgb_speed( this->rgb_speed( ) );
			( void )this->base_color_ref( );
			this->m_original_color = this->get_base_color( );
			this->sync_hsv_from_color( );
			this->sync_output_color( );
		}
		...
		bool process_input( const input_state& input ) override
		{
			...
			const auto anchor = this->get_current_anchor( );
			const auto popup = this->get_popup_rect( );
			const auto mode = this->current_mode( );
			const auto layout = this->get_layout( popup, mode );
			if ( mode == color_picker_mode::rainbow )
			{
				this->sync_output_color( );
			}
			...
			if ( input.mouse_clicked && anchor.contains( input.mouse_x, input.mouse_y ) )
			{
				this->m_closing = true;
				this->m_fully_closed = true;
				return true;
			...
			if ( input.mouse_clicked )
			{
				for ( int i = 0; i < 3; ++i )
				{
					if ( layout.tabs[ i ].contains( input.mouse_x, input.mouse_y ) )
					{
						this->set_mode( static_cast<color_picker_mode>( i ) );
						this->sync_hsv_from_color( );
						this->sync_output_color( );
				...
				if ( layout.show_content && layout.original_preview_rect.contains( input.mouse_x, input.mouse_y ) )
				{
					auto& base = this->base_color_ref( );
					base = this->m_original_color;
					this->sync_hsv_from_color( );
					this->sync_output_color( );
					this->m_active_component = 0;
					this->m_changed = true;
				...
				if ( mode == color_picker_mode::pulse )
				{
					return popup.contains( input.mouse_x, input.mouse_y );
				}

				if ( layout.show_content && layout.sv_rect.contains( input.mouse_x, input.mouse_y ) )
				{
					this->m_active_component = 1;
				}
				else if ( layout.show_content && layout.primary_bar_rect.contains( input.mouse_x, input.mouse_y ) )
				{
					this->m_active_component = 2;
				}
				else if ( layout.has_alpha && layout.alpha_rect.contains( input.mouse_x, input.mouse_y ) )
				{
			...
			if ( input.mouse_down && this->m_active_component != 0 )
			{
				if ( this->m_active_component == 1 )
				{
					if ( mode == color_picker_mode::rainbow )
					{
						this->m_hue = evaluate_rainbow_hue_degrees( this->rgb_speed( ), this->rgb_phase_offset( ) ) / 360.0f;
					}

					this->m_saturation = std::clamp( ( input.mouse_x - layout.sv_rect.x ) / layout.sv_rect.w, 0.0f, 1.0f );
					this->m_value = 1.0f - std::clamp( ( input.mouse_y - layout.sv_rect.y ) / layout.sv_rect.h, 0.0f, 1.0f );
					this->apply_hsv_to_color( );
				}
				else if ( this->m_active_component == 2 )
				{
					const auto t = std::clamp( ( input.mouse_y - layout.primary_bar_rect.y ) / layout.primary_bar_rect.h, 0.0f, 1.0f );
					if ( layout.primary_is_speed )
					{
						this->set_rgb_speed( k_rgb_speed_min + ( 1.0f - t ) * ( k_rgb_speed_max - k_rgb_speed_min ) );
					}
					else
					{
						this->m_hue = t;
				...
				{
					const auto alpha_norm = 1.0f - std::clamp( ( input.mouse_y - layout.alpha_rect.y ) / layout.alpha_rect.h, 0.0f, 1.0f );
					const auto alpha_value = static_cast< std::uint8_t >( alpha_norm * 255.0f );
					auto& base = this->base_color_ref( );
					base.a = alpha_value;
					this->sync_output_color( );
				}
			...
			if ( mode == color_picker_mode::rainbow && layout.primary_is_speed && layout.primary_bar_rect.contains( input.mouse_x, input.mouse_y ) )
			{
				const auto dt = zdraw::get_delta_time( );
				const auto& input_state = input;

				const auto key_pressed_or_edge = [ this, &input_state ]( int vk, std::unordered_map<int, bool>& state_map ) -> bool
					{
						...
						const auto msg_down = input_state.key_down_map.count( vk ) && input_state.key_down_map.at( vk );
						const auto async_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
						const auto is_down = msg_down || async_down;
						const auto was_down = state_map[ vk ];
						state_map[ vk ] = is_down;
						return is_down && !was_down;
					};

				const auto repeat_key = [ this, &input_state, dt ]( int vk, float initial_delay, float repeat_rate ) -> bool
					{
						const auto msg_down = input_state.key_down_map.count( vk ) && input_state.key_down_map.at( vk );
						const auto async_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
						const auto is_down = msg_down || async_down;
						const auto was_down = this->m_speed_key_was_down[ vk ];
						this->m_speed_key_was_down[ vk ] = is_down;

						for ( const auto pressed_key : input_state.key_presses( ) )
						{
							if ( pressed_key == vk )
							{
								this->m_speed_repeat_timers[ vk ] = 0.0f;
								return true;
							}
						}
						...
						this->m_speed_repeat_timers[ vk ] += dt;
						if ( this->m_speed_repeat_timers[ vk ] < initial_delay )
						{
							return false;
						}

						const auto excess = this->m_speed_repeat_timers[ vk ] - initial_delay;
						const auto repeats = static_cast< int >( excess / repeat_rate );
						const auto prev_repeats = static_cast< int >( ( excess - dt ) / repeat_rate );
						return repeats > prev_repeats;
					};

				auto speed_changed{ false };
				auto typed_changed{ false };

				const auto push_speed_char = [ this, &typed_changed ]( char ch )
					{
						if ( ch == ',' )
						{
							ch = '.';
						...
						{
							if ( this->m_speed_typed_buffer.size( ) < 16 )
							{
								this->m_speed_typed_buffer.push_back( ch );
								typed_changed = true;
							}
						...
						if ( ch == '.' && this->m_speed_typed_buffer.find( '.' ) == std::string::npos )
						{
							if ( this->m_speed_typed_buffer.empty( ) )
							{
								this->m_speed_typed_buffer.push_back( '0' );
							}

							this->m_speed_typed_buffer.push_back( '.' );
							typed_changed = true;
						}
					};

				for ( const auto wc : input_state.chars( ) )
				{
					if ( wc >= 0 && wc <= 255 )
					{
						push_speed_char( static_cast< char >( wc ) );
					}
				}

				if ( input_state.chars( ).empty( ) )
				{
					for ( auto vk = '0'; vk <= '9'; ++vk )
					{
						if ( key_pressed_or_edge( vk, this->m_speed_typed_key_was_down ) )
						{
							push_speed_char( static_cast< char >( vk ) );
						}
					}

					for ( auto vk = VK_NUMPAD0; vk <= VK_NUMPAD9; ++vk )
					{
						if ( key_pressed_or_edge( vk, this->m_speed_typed_key_was_down ) )
						{
							push_speed_char( static_cast< char >( '0' + ( vk - VK_NUMPAD0 ) ) );
						}
					}

					if ( key_pressed_or_edge( VK_OEM_COMMA, this->m_speed_typed_key_was_down ) ||
						key_pressed_or_edge( VK_OEM_PERIOD, this->m_speed_typed_key_was_down ) ||
						key_pressed_or_edge( VK_DECIMAL, this->m_speed_typed_key_was_down ) )
					{
						push_speed_char( '.' );
					}
				}

				if ( repeat_key( VK_BACK, 0.35f, 0.07f ) && !this->m_speed_typed_buffer.empty( ) )
				{
					this->m_speed_typed_buffer.pop_back( );
					typed_changed = true;
				}

				if ( repeat_key( VK_DELETE, 0.35f, 0.07f ) && !this->m_speed_typed_buffer.empty( ) )
				{
					this->m_speed_typed_buffer.pop_back( );
					typed_changed = true;
				}
				...
				if ( typed_changed )
				{
					this->m_speed_typed_idle_time = 0.0f;
					std::string parse = this->m_speed_typed_buffer;
					for ( auto& ch : parse )
					{
						if ( ch == ',' )
						{
					...
					auto parsed = std::strtof( parse.c_str( ), &end_ptr );
					if ( end_ptr != parse.c_str( ) && *end_ptr == '\0' )
					{
						this->set_rgb_speed( std::clamp( parsed, k_rgb_speed_min, k_rgb_speed_max ) );
						speed_changed = true;
					}
				}
				else
				{
					this->m_speed_typed_idle_time += dt;
					if ( this->m_speed_typed_idle_time > 1.1f )
					{
						this->m_speed_typed_buffer.clear( );
					}
				}

				const auto repeat_initial_delay = 0.32f;
				const auto repeat_rate = 0.065f;
				const auto dec = repeat_key( VK_LEFT, repeat_initial_delay, repeat_rate ) || repeat_key( 'A', repeat_initial_delay, repeat_rate );
				const auto inc = repeat_key( VK_RIGHT, repeat_initial_delay, repeat_rate ) || repeat_key( 'D', repeat_initial_delay, repeat_rate );

				if ( dec != inc )
				{
					const auto step = 0.1f;
					this->set_rgb_speed( this->rgb_speed( ) + ( inc ? step : -step ) );
					speed_changed = true;
				}
			...
			{
				this->m_speed_repeat_timers.clear( );
				this->m_speed_key_was_down.clear( );
				this->m_speed_typed_key_was_down.clear( );
				this->m_speed_typed_idle_time += zdraw::get_delta_time( );
				if ( this->m_speed_typed_idle_time > 1.1f )
				{
					this->m_speed_typed_buffer.clear( );
				}
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			const auto scaled_popup = zui::rect{ scaled_x, scaled_y, scaled_w, scaled_h };
			const auto mode = this->current_mode( );
			const auto layout = this->get_layout( scaled_popup, mode );

			this->render_tabs( style, input, layout, content_alpha );
			if ( layout.show_content )
			{
				this->render_sv_square( layout.sv_rect, content_alpha );
				this->render_primary_bar( style, layout, content_alpha );
				if ( layout.has_alpha )
				{
					this->render_alpha_bar( layout.alpha_rect, content_alpha );
				}
				this->render_preview_panel( style, layout, content_alpha, mode );
				this->render_value_rows( style, layout, content_alpha );
				this->render_cursors( layout, content_alpha );
			}
	...
	private:
		...
		{
			std::array<zui::rect, 3> tabs{};
			zui::rect sv_rect{};
			zui::rect primary_bar_rect{};
			zui::rect alpha_rect{};
			zui::rect current_preview_rect{};
			zui::rect original_preview_rect{};
			std::array<zui::rect, 4> rgba_cells{};
			std::array<zui::rect, 4> hsva_cells{};
			zui::rect hex_cell{};
			bool show_content{ false };
			bool has_alpha{ false };
			bool primary_is_speed{ false };
		};

		static constexpr float k_rgb_speed_min{ k_color_picker_rgb_speed_min };
		static constexpr float k_rgb_speed_max{ k_color_picker_rgb_speed_max };

		[[nodiscard]] int& mode_ref( )
		{
			auto& modes = get_color_picker_modes( );
			return modes.try_emplace( this->id( ), static_cast< int >( color_picker_mode::static_mode ) ).first->second;
		}

		[[nodiscard]] zdraw::rgba& base_color_ref( )
		{
			auto& colors = get_color_picker_base_colors( );
			if ( this->m_color_ptr )
			{
				return colors.try_emplace( this->id( ), *this->m_color_ptr ).first->second;
			}

			return colors.try_emplace( this->id( ), zdraw::rgba{ 255, 255, 255, 255 } ).first->second;
		}

		[[nodiscard]] float& rgb_speed_ref( )
		{
			auto& speeds = get_color_picker_rainbow_speeds( );
			return speeds.try_emplace( this->id( ), 1.0f ).first->second;
		}

		[[nodiscard]] float& rgb_phase_offset_ref( )
		{
			auto& phase_offsets = get_color_picker_rainbow_phase_offsets( );
			return phase_offsets.try_emplace( this->id( ), 0.0f ).first->second;
		}

		[[nodiscard]] float rgb_speed( ) const
		{
			auto& speeds = get_color_picker_rainbow_speeds( );
			const auto it = speeds.find( this->id( ) );
			if ( it == speeds.end( ) )
			{
				return 1.0f;
			}

			return std::clamp( it->second, k_rgb_speed_min, k_rgb_speed_max );
		}

		[[nodiscard]] float rgb_phase_offset( ) const
		{
			auto& phase_offsets = get_color_picker_rainbow_phase_offsets( );
			const auto it = phase_offsets.find( this->id( ) );
			if ( it == phase_offsets.end( ) )
			{
				return 0.0f;
			}
		...
		void set_rgb_speed( float speed )
		{
			const auto clamped = std::clamp( speed, k_rgb_speed_min, k_rgb_speed_max );
			auto& speed_ref = this->rgb_speed_ref( );
			const auto old_speed = std::clamp( speed_ref, k_rgb_speed_min, k_rgb_speed_max );

			if ( std::fabs( old_speed - clamped ) <= 0.0001f )
			{
				speed_ref = clamped;
				return;
			}

			auto& phase_offset = this->rgb_phase_offset_ref( );
			const auto time_s = get_color_picker_time_seconds( );
			phase_offset += time_s * 120.0f * ( old_speed - clamped );
			speed_ref = clamped;
		}

		[[nodiscard]] zdraw::rgba get_base_color( ) const
		{
			auto& colors = get_color_picker_base_colors( );
			const auto it = colors.find( this->id( ) );
			if ( it != colors.end( ) )
			{
				return it->second;
			}
			...
			if ( this->m_color_ptr )
			{
				return *this->m_color_ptr;
			}

			return zdraw::rgba{ 255, 255, 255, 255 };
		}

		[[nodiscard]] color_picker_mode current_mode( ) const
		{
			auto& modes = get_color_picker_modes( );
			const auto it = modes.find( this->id( ) );
			if ( it == modes.end( ) )
			{
				return color_picker_mode::static_mode;
			}

			return clamp_color_picker_mode( it->second );
		}

		void set_mode( color_picker_mode mode )
		{
			this->mode_ref( ) = static_cast< int >( mode );
		}

		void sync_hsv_from_color( )
		{
			const auto base = this->get_base_color( );
			const auto hsv_color = zui::rgb_to_hsv( base );
			this->m_hue = hsv_color.h / 360.0f;
			this->m_saturation = hsv_color.s;
			this->m_value = hsv_color.v;
		}

		void apply_hsv_to_color( )
		{
			auto& base = this->base_color_ref( );
		...
		void sync_output_color( )
		{
			...
			const auto base = this->get_base_color( );
			if ( this->current_mode( ) == color_picker_mode::rainbow )
			{
				*this->m_color_ptr = evaluate_color_picker_preview( color_picker_mode::rainbow, base, this->rgb_speed( ), this->rgb_phase_offset( ) );
			}
			else
			{
				*this->m_color_ptr = base;
			}
		}

		[[nodiscard]] picker_layout get_layout( const zui::rect& popup, color_picker_mode mode ) const
		{
			constexpr auto pad{ 8.0f };
			constexpr auto tab_spacing{ 2.0f };
			constexpr auto tab_h{ 22.0f };
			constexpr auto tab_gap{ 6.0f };
			constexpr auto alpha_bar_w{ 14.0f };
			constexpr auto primary_bar_w{ 14.0f };
			constexpr auto sv_to_bar_gap{ 4.0f };
			constexpr auto bar_to_bar_gap{ 4.0f };
			constexpr auto bars_to_preview_gap{ 6.0f };
			constexpr auto preview_w{ 64.0f };
			constexpr auto row_h{ 20.0f };
			constexpr auto row_gap{ 4.0f };

			picker_layout layout{};

			const auto tab_total_w = popup.w - pad * 2.0f;
			const auto tab_w = ( tab_total_w - tab_spacing * 2.0f ) / 3.0f;
			const auto tab_y = popup.y + pad;

			for ( int i = 0; i < 3; ++i )
			{
				layout.tabs[ i ] = zui::rect{ popup.x + pad + i * ( tab_w + tab_spacing ), tab_y, tab_w, tab_h };
			}

			const auto content_y = tab_y + tab_h + tab_gap;
			if ( mode == color_picker_mode::pulse )
			{
			...
			layout.show_content = true;
			layout.primary_is_speed = mode == color_picker_mode::rainbow;
			layout.has_alpha = this->m_show_alpha;

			const auto bars_w = primary_bar_w + ( layout.has_alpha ? ( bar_to_bar_gap + alpha_bar_w ) : 0.0f );
			auto sv_size = popup.w - pad * 2.0f - sv_to_bar_gap - bars_w - bars_to_preview_gap - preview_w;
			sv_size = std::max( sv_size, 120.0f );

			layout.sv_rect = zui::rect{ popup.x + pad, content_y, sv_size, sv_size };
			layout.primary_bar_rect = zui::rect{ layout.sv_rect.right( ) + sv_to_bar_gap, content_y, primary_bar_w, sv_size };
			if ( layout.has_alpha )
			{
				layout.alpha_rect = zui::rect{ layout.primary_bar_rect.right( ) + bar_to_bar_gap, content_y, alpha_bar_w, sv_size };
			}

			const auto preview_x = ( layout.has_alpha ? layout.alpha_rect.right( ) : layout.primary_bar_rect.right( ) ) + bars_to_preview_gap;
			constexpr auto preview_h{ 42.0f };
			constexpr auto preview_gap{ 4.0f };
			constexpr auto preview_label_h{ 14.0f };
			layout.current_preview_rect = zui::rect{ preview_x, content_y, preview_w, preview_h };
			layout.original_preview_rect = zui::rect
			{
				preview_x,
				layout.current_preview_rect.bottom( ) + preview_gap + preview_label_h + preview_gap,
				preview_w,
				preview_h
			};

			const auto rows_y = layout.sv_rect.bottom( ) + pad;
			const auto cell_w = ( popup.w - pad * 2.0f - row_gap * 3.0f ) / 4.0f;
			for ( int i = 0; i < 4; ++i )
			{
				layout.rgba_cells[ i ] = zui::rect{ popup.x + pad + i * ( cell_w + row_gap ), rows_y, cell_w, row_h };
				layout.hsva_cells[ i ] = zui::rect{ popup.x + pad + i * ( cell_w + row_gap ), rows_y + row_h + row_gap, cell_w, row_h };
			}
			layout.hex_cell = zui::rect{ popup.x + pad, rows_y + ( row_h + row_gap ) * 2.0f, popup.w - pad * 2.0f, row_h };
			return layout;
		}
		...
		[[nodiscard]] zui::rect get_popup_rect( ) const
		{
			...
			const auto width = this->m_show_alpha ? 330.0f : 306.0f;
			const auto bars_w = primary_bar_w + ( this->m_show_alpha ? ( bar_to_bar_gap + alpha_bar_w ) : 0.0f );
			const auto sv_size = width - pad * 2.0f - sv_to_bar_gap - bars_w - bars_to_preview_gap - preview_w;
			const auto height = pad + tab_h + tab_gap + sv_size + pad + row_h + row_gap + row_h + row_gap + row_h + pad;

			return zui::rect{ anchor.x, anchor.bottom( ) + 2.0f, width, height };
		}

		void render_tabs( const zui::style& style, const input_state& input, const picker_layout& layout, float alpha ) const
		{
			static constexpr std::array<const char*, 3> labels{ "Static", "Pulse", "RGB" };
			const auto current = this->current_mode( );
			const auto base = this->get_base_color( );
			const auto rgb_speed = this->rgb_speed( );

			for ( int i = 0; i < 3; ++i )
			{
				const auto hovered = layout.tabs[ i ].contains( input.mouse_x, input.mouse_y );
				const auto active = static_cast< int >( current ) == i;

				auto left_col = zui::darken( style.combo_bg, 0.95f );
				auto right_col = zui::darken( style.combo_bg, 0.9f );

				if ( active )
				{
					const auto preview = evaluate_color_picker_preview( static_cast<color_picker_mode>( i ), base, rgb_speed, this->rgb_phase_offset( ) );
					left_col = zui::lerp( style.combo_bg, preview, 0.12f );
					right_col = zui::darken( left_col, 0.75f );
				}
				else if ( hovered )
				{
					left_col = style.combo_hovered;
					right_col = zui::darken( style.combo_hovered, 0.86f );
				}

				left_col.a = static_cast< std::uint8_t >( left_col.a * alpha );
				right_col.a = static_cast< std::uint8_t >( right_col.a * alpha );

				auto border_col = style.combo_border;
				if ( active )
				{
					border_col = zui::lighten( border_col, 1.25f );
				}
				else if ( hovered )
				{
					border_col = zui::lighten( border_col, 1.1f );
				}

				border_col.a = static_cast< std::uint8_t >( border_col.a * alpha );

				zdraw::rect_filled_multi_color( layout.tabs[ i ].x, layout.tabs[ i ].y, layout.tabs[ i ].w, layout.tabs[ i ].h, left_col, right_col, right_col, left_col );
				zdraw::rect( layout.tabs[ i ].x, layout.tabs[ i ].y, layout.tabs[ i ].w, layout.tabs[ i ].h, border_col );
				if ( active )
				{
					auto accent = style.slider_fill;
					accent.a = static_cast< std::uint8_t >( accent.a * alpha );
					zdraw::rect_filled( layout.tabs[ i ].x, layout.tabs[ i ].bottom( ) - 2.0f, layout.tabs[ i ].w, 2.0f, accent );
				}

				auto text_col = style.text;
				if ( active )
				{
					text_col = zui::lighten( text_col, 1.15f );
				}
				else if ( hovered )
				{
					text_col = zui::lighten( text_col, 1.08f );
				}

				text_col.a = static_cast< std::uint8_t >( text_col.a * alpha );
				const auto text_size = zdraw::measure_text( labels[ i ] );
				const auto text_x = layout.tabs[ i ].x + ( layout.tabs[ i ].w - text_size.first ) * 0.5f;
				const auto text_y = layout.tabs[ i ].y + ( layout.tabs[ i ].h - text_size.second ) * 0.5f;
				zdraw::text( text_x, text_y, labels[ i ], text_col );
			}
		}

		void render_checkerboard( const zui::rect& r, float alpha ) const
		{
			constexpr auto cell{ 8.0f };
			for ( float y = 0.0f; y < r.h; y += cell )
			{
				for ( float x = 0.0f; x < r.w; x += cell )
				{
					const auto ix = static_cast< int >( x / cell );
					const auto iy = static_cast< int >( y / cell );
					auto col = ( ( ix + iy ) % 2 == 0 ) ? zdraw::rgba{ 160, 160, 160, 255 } : zdraw::rgba{ 210, 210, 210, 255 };
					col.a = static_cast< std::uint8_t >( col.a * alpha );
					zdraw::rect_filled( r.x + x, r.y + y, std::min( cell, r.w - x ), std::min( cell, r.h - y ), col );
				}
			}
		}
		...
		{
			auto render_hue = this->m_hue;
			if ( this->current_mode( ) == color_picker_mode::rainbow )
			{
				render_hue = evaluate_rainbow_hue_degrees( this->rgb_speed( ), this->rgb_phase_offset( ) ) / 360.0f;
			}
			...
			for ( int y = 0; y < static_cast< int >( r.h ); ++y )
			{
				for ( int x = 0; x < static_cast< int >( r.w ); ++x )
				{
					...
					auto color = zui::hsv_to_rgb( render_hue * 360.0f, s, v );
					color.a = static_cast< std::uint8_t >( color.a * alpha );
					zdraw::rect_filled( r.x + x, r.y + y, 1.0f, 1.0f, color );
				}
			}

			auto border_col = zdraw::rgba{ 45, 45, 45, static_cast< std::uint8_t >( 255 * alpha ) };
			zdraw::rect( r.x, r.y, r.w, r.h, border_col );
		}

		void render_primary_bar( const zui::style& style, const picker_layout& layout, float alpha ) const
		{
			const auto& r = layout.primary_bar_rect;
			if ( layout.primary_is_speed )
			{
				for ( int y = 0; y < static_cast< int >( r.h ); ++y )
				{
					const auto t = 1.0f - static_cast< float >( y ) / r.h;
					auto col = zui::lerp( zui::darken( style.slider_fill, 0.4f ), zui::lighten( style.slider_fill, 1.2f ), t );
					col.a = static_cast< std::uint8_t >( col.a * alpha );
					zdraw::rect_filled( r.x, r.y + y, r.w, 1.0f, col );
				}
			}
			else
			{
				for ( int y = 0; y < static_cast< int >( r.h ); ++y )
				{
					const auto h = ( static_cast< float >( y ) / r.h ) * 360.0f;
			...
			auto border_col = style.slider_border;
			border_col.a = static_cast< std::uint8_t >( border_col.a * alpha );
			zdraw::rect( r.x, r.y, r.w, r.h, border_col );
			...
			{
				char speed_text[ 16 ]{};
				std::snprintf( speed_text, sizeof( speed_text ), "%.1f", this->rgb_speed( ) );
				const auto [text_w, text_h] = zdraw::measure_text( speed_text );
				const auto text_x = r.x + ( r.w - text_w ) * 0.5f;
				const auto text_y = r.y + ( r.h - text_h ) * 0.5f;

				auto text_col = zui::lighten( style.text, 1.1f );
				text_col.a = static_cast< std::uint8_t >( text_col.a * alpha );
				zdraw::text( text_x, text_y, speed_text, text_col );
			}
		}

		void render_alpha_bar( const zui::rect& r, float alpha ) const
		{
			this->render_checkerboard( r, alpha );
			const auto base = this->get_base_color( );
			for ( int y = 0; y < static_cast< int >( r.h ); ++y )
			{
				const auto a = static_cast< std::uint8_t >( ( 1.0f - static_cast< float >( y ) / r.h ) * 255.0f * alpha );
				const auto color = zdraw::rgba{ base.r, base.g, base.b, a };
				zdraw::rect_filled( r.x, r.y + y, r.w, 1.0f, color );
			}
			auto border_col = zdraw::rgba{ 45, 45, 45, static_cast< std::uint8_t >( 255 * alpha ) };
			zdraw::rect( r.x, r.y, r.w, r.h, border_col );
		...
		void render_preview_panel( const zui::style& style, const picker_layout& layout, float alpha, color_picker_mode mode ) const
		{
			const auto base = this->get_base_color( );
			auto current = base;
			if ( mode == color_picker_mode::rainbow )
			{
				current = evaluate_color_picker_preview( mode, base, this->rgb_speed( ), this->rgb_phase_offset( ) );
			}

			this->render_checkerboard( layout.current_preview_rect, alpha );
			auto current_col = current;
			current_col.a = static_cast< std::uint8_t >( current_col.a * alpha );
			zdraw::rect_filled( layout.current_preview_rect.x, layout.current_preview_rect.y, layout.current_preview_rect.w, layout.current_preview_rect.h, current_col );
			zdraw::rect( layout.current_preview_rect.x, layout.current_preview_rect.y, layout.current_preview_rect.w, layout.current_preview_rect.h, zdraw::rgba{ 50, 50, 50, static_cast< std::uint8_t >( 255 * alpha ) } );

			this->render_checkerboard( layout.original_preview_rect, alpha );
			auto original_col = this->m_original_color;
			original_col.a = static_cast< std::uint8_t >( original_col.a * alpha );
			zdraw::rect_filled( layout.original_preview_rect.x, layout.original_preview_rect.y, layout.original_preview_rect.w, layout.original_preview_rect.h, original_col );
			zdraw::rect( layout.original_preview_rect.x, layout.original_preview_rect.y, layout.original_preview_rect.w, layout.original_preview_rect.h, zdraw::rgba{ 50, 50, 50, static_cast< std::uint8_t >( 255 * alpha ) } );

			auto text_col = style.text;
			text_col.a = static_cast< std::uint8_t >( text_col.a * alpha );
			zdraw::text( layout.current_preview_rect.x, layout.current_preview_rect.bottom( ) + 3.0f, "Original", text_col );
		}

		void render_value_cell( const zui::style& style, const zui::rect& r, const char* text, float alpha ) const
		{
			auto bg = zui::darken( style.combo_bg, 0.95f );
			bg.a = static_cast< std::uint8_t >( bg.a * alpha );
			auto border = style.combo_border;
			border.a = static_cast< std::uint8_t >( border.a * alpha );
			auto txt = style.text;
			txt.a = static_cast< std::uint8_t >( txt.a * alpha );

			zdraw::rect_filled( r.x, r.y, r.w, r.h, bg );
			zdraw::rect( r.x, r.y, r.w, r.h, border );
			const auto text_size = zdraw::measure_text( text );
			zdraw::text( r.x + 6.0f, r.y + ( r.h - text_size.second ) * 0.5f, text, txt );
		}

		void render_value_rows( const zui::style& style, const picker_layout& layout, float alpha ) const
		{
			const auto base = this->get_base_color( );
			const auto hsv = zui::rgb_to_hsv( base );

			char rgba_text[ 4 ][ 16 ]{};
			char hsva_text[ 4 ][ 16 ]{};
			char hex_text[ 16 ]{};

			std::snprintf( rgba_text[ 0 ], sizeof( rgba_text[ 0 ] ), "R: %d", base.r );
			std::snprintf( rgba_text[ 1 ], sizeof( rgba_text[ 1 ] ), "G: %d", base.g );
			std::snprintf( rgba_text[ 2 ], sizeof( rgba_text[ 2 ] ), "B: %d", base.b );
			std::snprintf( rgba_text[ 3 ], sizeof( rgba_text[ 3 ] ), "A: %d", base.a );

			std::snprintf( hsva_text[ 0 ], sizeof( hsva_text[ 0 ] ), "H: %d", static_cast< int >( std::round( hsv.h ) ) );
			std::snprintf( hsva_text[ 1 ], sizeof( hsva_text[ 1 ] ), "S: %d", static_cast< int >( std::round( hsv.s * 255.0f ) ) );
			std::snprintf( hsva_text[ 2 ], sizeof( hsva_text[ 2 ] ), "V: %d", static_cast< int >( std::round( hsv.v * 255.0f ) ) );
			std::snprintf( hsva_text[ 3 ], sizeof( hsva_text[ 3 ] ), "A: %d", base.a );

			std::snprintf( hex_text, sizeof( hex_text ), "#%02X%02X%02X%02X", base.r, base.g, base.b, base.a );

			for ( int i = 0; i < 4; ++i )
			{
				this->render_value_cell( style, layout.rgba_cells[ i ], rgba_text[ i ], alpha );
				this->render_value_cell( style, layout.hsva_cells[ i ], hsva_text[ i ], alpha );
			}

			this->render_value_cell( style, layout.hex_cell, hex_text, alpha );
		}

		void render_cursors( const picker_layout& layout, float content_alpha ) const
		{
			...
			const auto sv_x = layout.sv_rect.x + this->m_saturation * layout.sv_rect.w;
			const auto sv_y = layout.sv_rect.y + ( 1.0f - this->m_value ) * layout.sv_rect.h;
			zdraw::circle_filled( sv_x, sv_y, 5.0f, black_col, 16 );
			zdraw::circle_filled( sv_x, sv_y, 4.0f, white_col, 16 );

			float primary_t = this->m_hue;
			if ( layout.primary_is_speed )
			{
				primary_t = 1.0f - ( this->rgb_speed( ) - k_rgb_speed_min ) / ( k_rgb_speed_max - k_rgb_speed_min );
			}
			const auto primary_y = layout.primary_bar_rect.y + primary_t * layout.primary_bar_rect.h;
			zdraw::triangle_filled( layout.primary_bar_rect.x - 5.0f, primary_y, layout.primary_bar_rect.x - 1.0f, primary_y - 4.0f, layout.primary_bar_rect.x - 1.0f, primary_y + 4.0f, white_col );
			zdraw::triangle_filled( layout.primary_bar_rect.right( ) + 5.0f, primary_y, layout.primary_bar_rect.right( ) + 1.0f, primary_y - 4.0f, layout.primary_bar_rect.right( ) + 1.0f, primary_y + 4.0f, white_col );

			if ( layout.has_alpha )
			{
				const auto base = this->get_base_color( );
				const auto alpha_y = layout.alpha_rect.y + ( 1.0f - base.a / 255.0f ) * layout.alpha_rect.h;
				zdraw::triangle_filled( layout.alpha_rect.x - 5.0f, alpha_y, layout.alpha_rect.x - 1.0f, alpha_y - 4.0f, layout.alpha_rect.x - 1.0f, alpha_y + 4.0f, white_col );
				zdraw::triangle_filled( layout.alpha_rect.right( ) + 5.0f, alpha_y, layout.alpha_rect.right( ) + 1.0f, alpha_y - 4.0f, layout.alpha_rect.right( ) + 1.0f, alpha_y + 4.0f, white_col );
			}
		}

		zdraw::rgba* m_color_ptr{ nullptr };
		zdraw::rgba m_original_color{ 255, 255, 255, 255 };
		float m_hue{ 0.0f };
		...
		std::string m_speed_typed_buffer{};
		float m_speed_typed_idle_time{ 0.0f };
		std::unordered_map<int, float> m_speed_repeat_timers{};
		std::unordered_map<int, bool> m_speed_key_was_down{};
		std::unordered_map<int, bool> m_speed_typed_key_was_down{};
	};
	...
	public:
		...
		void begin_frame( )
		{
			...
			this->m_scroll_wheel_consumed = false;
		}

		void end_frame( )
		{
			const auto insert_down = ( ::GetAsyncKeyState( VK_INSERT ) & 0x8000 ) != 0;
			const auto insert_pressed = insert_down && !this->m_insert_was_down;
			this->m_insert_was_down = insert_down;

			if ( insert_pressed )
			{
				this->m_active_text_input_id = invalid_id;
			}
		...
		[[nodiscard]] int get_keybind_mode_value( std::uintptr_t storage ) noexcept
		{
			const auto [it, inserted] = this->m_keybind_mode_map.try_emplace( storage, keybind_mode_hold );
			if ( inserted )
			{
				this->m_keybind_toggle_state.try_emplace( storage, false );
				this->m_keybind_prev_down.try_emplace( storage, false );
			}

			it->second = normalize_keybind_mode( it->second );
			return it->second;
		}

		void set_keybind_mode_value( std::uintptr_t storage, int mode ) noexcept
		{
			const auto normalized = normalize_keybind_mode( mode );
			this->m_keybind_mode_map[ storage ] = normalized;

			if ( normalized != keybind_mode_toggle )
			{
				this->m_keybind_toggle_state[ storage ] = false;
			}
		}

		void set_keybind_mode_locked( std::uintptr_t storage, bool locked ) noexcept
		{
			this->m_keybind_mode_locked[ storage ] = locked;
		}

		[[nodiscard]] bool is_keybind_mode_locked( std::uintptr_t storage ) noexcept
		{
			const auto it = this->m_keybind_mode_locked.find( storage );
			if ( it == this->m_keybind_mode_locked.end( ) )
			{
				return false;
			}
		...
		[[nodiscard]] bool keybind_active( std::uintptr_t storage, int key ) noexcept
		{
			const auto mode = this->get_keybind_mode_value( storage );
			auto& prev_down = this->m_keybind_prev_down[ storage ];
			auto& toggled = this->m_keybind_toggle_state[ storage ];
			...
			const auto is_down = ( ::GetAsyncKeyState( key ) & 0x8000 ) != 0;
			if ( mode == keybind_mode_hold )
			{
				prev_down = is_down;
				return is_down;
			}
		...
		std::unordered_map<std::uintptr_t, int> m_keybind_mode_map{};
		std::unordered_map<std::uintptr_t, bool> m_keybind_mode_locked{};
		std::unordered_map<std::uintptr_t, bool> m_keybind_toggle_state{};
		std::unordered_map<std::uintptr_t, bool> m_keybind_prev_down{};
		bool m_scroll_wheel_consumed{ false };
...
namespace zui {
	...
	void update_dynamic_color_pickers( )
	{
		auto& modes = get_color_picker_modes( );
		auto& base_colors = get_color_picker_base_colors( );
		auto& speeds = get_color_picker_rainbow_speeds( );
		auto& phase_offsets = get_color_picker_rainbow_phase_offsets( );
		auto& targets = get_color_picker_targets( );

		for ( const auto& [id, target_ptr] : targets )
		{
			if ( !target_ptr )
			{
				continue;
			...
			const auto base_it = base_colors.find( id );
			if ( base_it == base_colors.end( ) )
			{
				continue;
			}

			const auto mode_it = modes.find( id );
			const auto mode = mode_it != modes.end( ) ? clamp_color_picker_mode( mode_it->second ) : color_picker_mode::static_mode;

			if ( mode == color_picker_mode::rainbow )
			{
				const auto speed_it = speeds.find( id );
				const auto speed = speed_it != speeds.end( ) ? std::clamp( speed_it->second, k_color_picker_rgb_speed_min, k_color_picker_rgb_speed_max ) : 1.0f;
				const auto phase_it = phase_offsets.find( id );
				const auto phase_offset = phase_it != phase_offsets.end( ) ? phase_it->second : 0.0f;
				*target_ptr = evaluate_color_picker_preview( mode, base_it->second, speed, phase_offset );
			}
			else
			{
				*target_ptr = base_it->second;
			}
		}
	}
	...
	namespace
	{
		...
		void process_scroll_wheel( const rect& bounds, float content_height, float visible_start_y, float& scroll_y )
		{
			...
			const auto visible_height = std::max( 0.0f, bounds.h - visible_start_y );
			const auto effective_content_height = std::max( 0.0f, content_height - visible_start_y ) + style.window_padding_y;
			const auto overflow = effective_content_height - visible_height;
			const auto max_scroll = overflow > 1.0f ? overflow : 0.0f;
			...
			const auto hovered = input.hovered( bounds ) && !ctx( ).overlay_blocking_input( );
			const auto scroll_delta = input.scroll_delta( );

			if ( hovered && !ctx( ).m_scroll_wheel_consumed && scroll_delta != 0.0f )
			{
				scroll_y -= scroll_delta * 30.0f;
				ctx( ).m_scroll_wheel_consumed = true;
			}
	...
	bool begin_window( std::string_view title, float& x, float& y, float& w, float& h, bool resizable, float min_w, float min_h )
	{
		...
		const auto clip_l = abs.x + 1.0f;
		const auto clip_t = abs.y + 1.0f;
		const auto clip_r = std::max( clip_l, abs.right( ) - 1.0f );
		const auto clip_b = std::max( clip_t, abs.bottom( ) - 1.0f );
		zdraw::push_clip_rect( clip_l, clip_t, clip_r, clip_b );
		ctx( ).push_id( id );
	...
	bool begin_group_box( std::string_view title, float w, float h )
	{
		...
		state.visible_start_y = title_h;
		state.scroll_id = id;

		ctx( ).push_window( std::move( state ) );
		const auto clip_l = abs.x + 1.0f;
		const auto clip_t = abs.y + title_h + 1.0f;
		const auto clip_r = std::max( clip_l, abs.right( ) - 1.0f );
		const auto clip_b = std::max( clip_t, abs.bottom( ) - 1.0f );
		zdraw::push_clip_rect( clip_l, clip_t, clip_r, clip_b );
		ctx( ).push_id( id );
	...
	namespace
	{
		struct slider_keyboard_state
		{
			std::string typed_buffer{};
			float typed_idle_time{ 0.0f };
			std::unordered_map<int, float> key_repeat_timers{};
			std::unordered_map<int, bool> key_was_down{};
			std::unordered_map<int, bool> typed_key_was_down{};
		};

		std::unordered_map<widget_id, slider_keyboard_state>& get_slider_keyboard_states( )
		{
			static std::unordered_map<widget_id, slider_keyboard_state> states{};
			return states;
		}

		[[nodiscard]] int slider_float_precision_from_format( std::string_view format )
		{
			...
			for ( std::size_t i = dot + 1; i < format.size( ); ++i )
			{
				const auto ch = static_cast< unsigned char >( format[ i ] );
				if ( std::isdigit( ch ) )
				{
					found_digit = true;
			...
			return found_digit ? std::clamp( precision, 0, 4 ) : 0;
		}

		[[nodiscard]] float slider_step_from_precision( int precision )
		{
			const auto clamped = std::clamp( precision, 0, 4 );
			return std::pow( 10.0f, -static_cast< float >( clamped ) );
		}

		bool slider_key_pressed_or_edge_async( std::unordered_map<int, bool>& was_down_map, const input_state& input_state, int vk )
		{
			for ( const auto pressed_key : input_state.key_presses( ) )
			{
				if ( pressed_key == vk )
				{
					was_down_map[ vk ] = true;
					return true;
				}
			}
			...
			const auto async_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
			const auto is_down = msg_down || async_down;
			const auto was_down = was_down_map[ vk ];
			was_down_map[ vk ] = is_down;

			return is_down && !was_down;
		}

		bool process_slider_repeat_key( slider_keyboard_state& state, const input_state& input_state, int vk, float dt, float initial_delay, float repeat_rate )
		{
			const auto msg_down = input_state.key_down_map.count( vk ) && input_state.key_down_map.at( vk );
			const auto async_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
			const auto is_down = msg_down || async_down;
		...
		template<typename T>
		bool apply_slider_typed_value( std::string_view typed, T& v, T v_min, T v_max, int float_precision )
		{
			...
			{
				std::string parse{ typed };
				char* end_ptr{ nullptr };
				const auto parsed = std::strtol( parse.c_str( ), &end_ptr, 10 );
				if ( end_ptr == parse.c_str( ) || *end_ptr != '\0' )
				{
					return false;
				}

				const auto next = static_cast< T >( std::clamp( parsed, static_cast< long >( v_min ), static_cast< long >( v_max ) ) );
				if ( next == v )
				{
					return false;
				}
			...
			else
			{
				...
				parsed = std::clamp( parsed, static_cast< float >( v_min ), static_cast< float >( v_max ) );

				const auto next = static_cast< T >( parsed );
				if ( std::fabs( next - v ) <= 0.0001f )
				{
					return false;
				}
		...
		template<typename T>
		bool apply_slider_key_step( T& v, T v_min, T v_max, float step, bool increase, int float_precision )
		{
			if constexpr ( std::is_integral_v<T> )
			{
				const auto delta = increase ? 1 : -1;
				const auto next = static_cast< T >( std::clamp( static_cast< int >( v ) + delta, static_cast< int >( v_min ), static_cast< int >( v_max ) ) );
				if ( next == v )
				{
					return false;
				}
			...
			{
				auto next = static_cast< float >( v ) + ( increase ? step : -step );
				next = std::clamp( next, static_cast< float >( v_min ), static_cast< float >( v_max ) );
				const auto scale = std::pow( 10.0f, static_cast< float >( std::clamp( float_precision, 0, 4 ) ) );
				next = std::round( next * scale ) / scale;
		...
		bool slider_impl( std::string_view label, T& v, T v_min, T v_max, std::string_view format )
		{
			...
			const auto hover_rect = rect{ abs.x, abs.y, slider_width, total_height };
			const auto hit_rect = rect{ abs.x, knob_y, slider_width, knob_height };
			const auto can_interact = !ctx( ).overlay_blocking_input( );
			const auto hovered_track = can_interact && input.hovered( hit_rect );
			const auto hovered = can_interact && input.hovered( hover_rect );

			if ( hovered_track && input.mouse_clicked( ) && ctx( ).m_active_slider_id == invalid_id )
			{
			...
			auto& slider_key_states = get_slider_keyboard_states( );
			auto& key_state = slider_key_states[ id ];
			const auto dt = zdraw::get_delta_time( );
			const auto allow_keyboard_input = hovered && can_interact;
			const auto float_precision = std::is_floating_point_v<T> ? slider_float_precision_from_format( format ) : 0;
			const auto step = std::is_integral_v<T> ? 1.0f : slider_step_from_precision( float_precision );

			if ( allow_keyboard_input )
			{
				const auto& input_state = input.current( );
				auto has_typed_edit{ false };
				if ( key_state.typed_idle_time > 1.1f )
				{
					key_state.typed_buffer.clear( );
				}

				const auto typed_chars = input_state.chars( );
				const auto push_char_to_buffer = [ & ]( char ch )
					{
						if ( ch == ',' )
						{
							ch = '.';
						...
						if ( std::isdigit( static_cast< unsigned char >( ch ) ) )
						{
							if ( key_state.typed_buffer.size( ) < 24 )
							{
								key_state.typed_buffer.push_back( ch );
								has_typed_edit = true;
							}
						...
						if constexpr ( std::is_floating_point_v<T> )
						{
							if ( ch == '.' && key_state.typed_buffer.find( '.' ) == std::string::npos )
							{
								if ( key_state.typed_buffer.empty( ) || key_state.typed_buffer == "-" )
								{
									key_state.typed_buffer.push_back( '0' );
								}

								key_state.typed_buffer.push_back( '.' );
								has_typed_edit = true;
								return;
							}
						}

						if ( ch == '-' && static_cast< float >( v_min ) < 0.0f && key_state.typed_buffer.empty( ) )
						{
							key_state.typed_buffer.push_back( '-' );
							has_typed_edit = true;
						}
				...
				for ( const auto wc : typed_chars )
				{
					char ch{ '\0' };
					if ( wc >= 0 && wc <= 255 )
					{
						ch = static_cast< char >( wc );
					}

					push_char_to_buffer( ch );
				}

				for ( auto vk = '0'; vk <= '9'; ++vk )
				{
					if ( input_state.key_down_map.count( vk ) && input_state.key_down_map.at( vk ) )
					{
						key_state.typed_key_was_down[ vk ] = true;
					}
				}
				...
				if ( typed_chars.empty( ) )
				{
					for ( auto vk = '0'; vk <= '9'; ++vk )
					{
						if ( slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, vk ) )
						{
							push_char_to_buffer( static_cast< char >( vk ) );
						}
					}
					...
					for ( auto vk = VK_NUMPAD0; vk <= VK_NUMPAD9; ++vk )
					{
						if ( slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, vk ) )
						{
							push_char_to_buffer( static_cast< char >( '0' + ( vk - VK_NUMPAD0 ) ) );
						}
					}

					if ( slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, VK_OEM_COMMA ) ||
						slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, VK_OEM_PERIOD ) ||
						slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, VK_DECIMAL ) )
					{
						push_char_to_buffer( '.' );
					}

					if ( slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, VK_OEM_MINUS ) ||
						slider_key_pressed_or_edge_async( key_state.typed_key_was_down, input_state, VK_SUBTRACT ) )
					{
						push_char_to_buffer( '-' );
					}
				}

				if ( process_slider_repeat_key( key_state, input_state, VK_BACK, dt, 0.35f, 0.07f ) && !key_state.typed_buffer.empty( ) )
				{
					key_state.typed_buffer.pop_back( );
					has_typed_edit = true;
				}

				if ( process_slider_repeat_key( key_state, input_state, VK_DELETE, dt, 0.35f, 0.07f ) && !key_state.typed_buffer.empty( ) )
				{
					key_state.typed_buffer.pop_back( );
					has_typed_edit = true;
				}
				...
				if ( has_typed_edit )
				{
					key_state.typed_idle_time = 0.0f;
					changed = apply_slider_typed_value( key_state.typed_buffer, v, v_min, v_max, float_precision ) || changed;
				}
				else
				{
					key_state.typed_idle_time += dt;
					if ( key_state.typed_idle_time > 1.1f )
					{
						key_state.typed_buffer.clear( );
					}
				...
				const auto repeat_initial_delay = 0.32f;
				const auto repeat_rate = std::is_integral_v<T> ? 0.09f : ( float_precision >= 2 ? 0.05f : ( float_precision == 1 ? 0.065f : 0.085f ) );

				const auto dec_pressed = process_slider_repeat_key( key_state, input_state, VK_LEFT, dt, repeat_initial_delay, repeat_rate ) || process_slider_repeat_key( key_state, input_state, 'A', dt, repeat_initial_delay, repeat_rate );
				const auto inc_pressed = process_slider_repeat_key( key_state, input_state, VK_RIGHT, dt, repeat_initial_delay, repeat_rate ) || process_slider_repeat_key( key_state, input_state, 'D', dt, repeat_initial_delay, repeat_rate );

				if ( dec_pressed != inc_pressed )
				{
					changed = apply_slider_key_step( v, v_min, v_max, step, inc_pressed, float_precision ) || changed;
				}
			}
			else
			{
				key_state.key_repeat_timers.clear( );
				key_state.key_was_down.clear( );
				key_state.typed_key_was_down.clear( );
				key_state.typed_idle_time += dt;
				if ( key_state.typed_idle_time > 1.1f )
				{
					key_state.typed_buffer.clear( );
			...
			const auto render_value_w = zdraw::measure_text( value_buf ).first;
			...
			if ( !display_label.empty( ) )
			{
				const auto available_label_w = slider_width - render_value_w - style.item_spacing_x;
			...
			zdraw::text( abs.x + slider_width - render_value_w, abs.y, value_buf, value_col );
	...
	class keybind_mode_overlay : public overlay
	{
	public:
		keybind_mode_overlay( widget_id id, const zui::rect& anchor, std::uintptr_t storage ) : overlay{ id, anchor }, m_storage{ storage } { }
		...
		bool process_input( const input_state& input ) override
		{
			if ( this->m_skip_first_input_frame )
			{
				// Ignore the opening click frame to avoid instant close right after spawning.
				this->m_skip_first_input_frame = false;
				return true;
			}
			...
			const auto popup = this->get_popup_rect( );
			const auto anchor = this->get_current_anchor( );
			const auto any_click = input.mouse_clicked || input.right_mouse_clicked;

			if ( any_click && !popup.contains( input.mouse_x, input.mouse_y ) && !anchor.contains( input.mouse_x, input.mouse_y ) )
			{
				this->m_closing = true;
				this->m_fully_closed = true;
				return true;
			...
			if ( any_click && anchor.contains( input.mouse_x, input.mouse_y ) )
			{
				this->m_closing = true;
				this->m_fully_closed = true;
				return true;
			...
			if ( input.mouse_clicked && popup.contains( input.mouse_x, input.mouse_y ) )
			{
				constexpr std::array<int, 3> option_modes{ keybind_mode_always_on, keybind_mode_toggle, keybind_mode_hold };
				const auto rows = this->build_row_rects( popup );
				for ( std::size_t i = 0; i < rows.size( ); ++i )
				{
					if ( !rows[ i ].contains( input.mouse_x, input.mouse_y ) )
					{
						continue;
					}

					ctx( ).set_keybind_mode_value( this->m_storage, option_modes[ i ] );
					this->m_closing = true;
					this->m_fully_closed = true;
					return true;
				}
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			auto border = style.combo_popup_border;
			zdraw::rect_filled( popup.x, popup.y, popup.w, popup.h, bg );
			zdraw::rect( popup.x, popup.y, popup.w, popup.h, border );

			constexpr std::array<const char*, 3> option_labels{ "Always on", "Toggle", "Hold" };
			constexpr std::array<int, 3> option_modes{ keybind_mode_always_on, keybind_mode_toggle, keybind_mode_hold };
			const auto rows = this->build_row_rects( popup );
			const auto current_mode = ctx( ).get_keybind_mode_value( this->m_storage );

			for ( std::size_t i = 0; i < rows.size( ); ++i )
			{
				const auto hovered = rows[ i ].contains( input.mouse_x, input.mouse_y );
				const auto selected = current_mode == option_modes[ i ];
				if ( hovered || selected )
				{
					auto fill = hovered ? style.combo_item_hovered : style.combo_item_selected;
					zdraw::rect_filled( rows[ i ].x, rows[ i ].y, rows[ i ].w, rows[ i ].h, fill );
				}
				...
				auto [text_w, th] = zdraw::measure_text( option_labels[ i ] );
				( void )text_w;
				const auto tx = rows[ i ].x + 8.0f;
				const auto ty = rows[ i ].y + ( rows[ i ].h - th ) * 0.5f;
				zdraw::text( tx, ty, option_labels[ i ], text_col );
			}
		}

		[[nodiscard]] bool should_close( ) const override
		{
			return this->m_fully_closed || this->m_closing;
		}

		[[nodiscard]] bool is_closed( ) const override
		{
			return this->m_fully_closed;
		}

	private:
		[[nodiscard]] static std::array<zui::rect, 3> build_row_rects( const zui::rect& popup )
		{
			constexpr auto pad{ 4.0f };
			constexpr auto row_h{ 20.0f };
			constexpr auto row_gap{ 2.0f };

			std::array<zui::rect, 3> rows{};
			auto y = popup.y + pad;
			for ( auto& row : rows )
			{
				row = zui::rect{ popup.x + pad, y, popup.w - pad * 2.0f, row_h };
				y += row_h + row_gap;
			}
		...
		[[nodiscard]] zui::rect get_popup_rect( ) const
		{
			constexpr auto width{ 124.0f };
			constexpr auto height{ 72.0f };
			auto anchor = this->get_current_anchor( );
			auto x = anchor.x;
			auto y = anchor.bottom( ) + 2.0f;

			const auto [display_w, display_h] = zdraw::get_display_size( );
			x = std::clamp( x, 2.0f, std::max( 2.0f, static_cast<float>( display_w ) - width - 2.0f ) );
			y = std::clamp( y, 2.0f, std::max( 2.0f, static_cast<float>( display_h ) - height - 2.0f ) );

			return zui::rect{ x, y, width, height };
		}

		std::uintptr_t m_storage{};
		bool m_closing{ false };
		bool m_fully_closed{ false };
		bool m_skip_first_input_frame{ true };
	};
		...
		const auto& input = ctx( ).input( );
			...
			const auto key_storage = reinterpret_cast< std::uintptr_t >( &key );
			const auto mode_overlay_id = id ^ 0x9f6a06f31be4f8d3ull;
			const auto mode_locked = ctx( ).is_keybind_mode_locked( key_storage );

			const auto is_waiting = ( ctx( ).m_active_keybind_id == id );
			( void )ctx( ).get_keybind_mode_value( key_storage );
			static std::array<bool, 256> async_prev_down{};
			static widget_id async_capture_owner{ invalid_id };

			const auto sync_async_state = [ & ]( )
				{
					for ( auto vk = 1; vk < 256; ++vk )
					{
						async_prev_down[ vk ] = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
					}
				};

			if ( is_waiting )
			{
				if ( async_capture_owner != id )
				{
					async_capture_owner = id;
					sync_async_state( );
				}
			}
			else if ( async_capture_owner == id )
			{
				async_capture_owner = invalid_id;
			}

			const auto commit_key = [ & ]( int vk ) -> bool
				{
		...
		if ( auto* mode_popup = dynamic_cast< keybind_mode_overlay* >( overlays.find( mode_overlay_id ) ) )
		{
			mode_popup->update_anchor( button_rect );
		}
		...
		if ( pressed )
		{
			overlays.close( mode_overlay_id );
			ctx( ).m_active_keybind_id = id;
		}

		if ( mode_locked && overlays.is_open( mode_overlay_id ) )
		{
			overlays.close( mode_overlay_id );
		}

		const auto right_pressed = hovered && input.right_mouse_clicked( );
		if ( right_pressed && !is_waiting && !mode_locked )
		{
			if ( overlays.is_open( mode_overlay_id ) )
			{
				overlays.close( mode_overlay_id );
			}
			else
			{
				overlays.add<keybind_mode_overlay>( mode_overlay_id, button_rect, key_storage );
			}
		}
		...
		if ( !display_label.empty( ) )
		{
			...
			if ( is_waiting )
			{
				...
				{
					return commit_key( VK_LBUTTON );
				}

				if ( input.right_mouse_clicked( ) )
				{
					return commit_key( VK_RBUTTON );
				}
				...
				{
					return commit_key( key_presses.front( ) );
				}

				// Fallback for non-focused overlay: detect edge-triggered keys globally.
				for ( auto vk = 1; vk < 256; ++vk )
				{
					const auto is_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
					const auto was_down = async_prev_down[ vk ];
					async_prev_down[ vk ] = is_down;
	...
	bool keybind_active( const int& key )
	{
		const auto storage = reinterpret_cast< std::uintptr_t >( &key );
		return ctx( ).keybind_active( storage, key );
	}

	int keybind_mode_value( const int& key )
	{
		const auto storage = reinterpret_cast< std::uintptr_t >( &key );
		return ctx( ).get_keybind_mode_value( storage );
	}

	void set_keybind_mode_value( const int& key, int mode )
	{
		const auto storage = reinterpret_cast< std::uintptr_t >( &key );
		ctx( ).set_keybind_mode_value( storage, mode );
	}

	void set_keybind_mode_locked( const int& key, bool locked )
	{
		const auto storage = reinterpret_cast< std::uintptr_t >( &key );
		ctx( ).set_keybind_mode_locked( storage, locked );
	}
	...
	bool combo( std::string_view label, int& current_item, const char* const items[ ], int items_count, float width )
	{
		...
		auto& combo_values = get_combo_values( );
		auto [combo_it, combo_inserted] = combo_values.try_emplace( id, current_item );
		auto& selected_item = combo_it->second;
		auto& combo_dirty_states = get_combo_dirty_states( );
		auto& combo_dirty = combo_dirty_states.try_emplace( id, false ).first->second;

		if ( !is_open && !combo_inserted && !combo_dirty )
		{
		...
		if ( hovered && input.mouse_clicked( ) )
		{
			const auto open_now = overlays.is_open( id );
			if ( open_now )
			{
				overlays.close( id );
			}
			else if ( !ctx( ).overlay_blocking_input( ) )
			{
				...
				overlays.add<combo_overlay>(
					...
					[ id ]( )
					{
						get_combo_dirty_states( )[ id ] = true;
					}
				);
			}
		}
		...
		const auto current_text = ( selected_item >= 0 && selected_item < items_count ) ? items[ selected_item ] : "";
		const auto [text_w, text_h] = zdraw::measure_text( current_text );
		const auto text_x = button_rect.x + style.frame_padding_x + 6.0f;
	...
	bool multicombo( std::string_view label, bool* selected_items, const char* const items[ ], int items_count, float width, bool require_one_selected )
	{
		const auto win = ctx( ).current_window( );
		...
		const auto is_open = overlays.is_open( id );
		auto& multicombo_values = get_multicombo_values( );
		auto values_it = multicombo_values.try_emplace( id ).first;
		auto& selected_state = values_it->second;

		if ( selected_state.size( ) != static_cast< std::size_t >( items_count ) )
		{
			selected_state.assign( items_count, 0u );
			for ( int i = 0; i < items_count; ++i )
			{
				selected_state[ i ] = selected_items[ i ] ? 1u : 0u;
			}
		}
		else if ( !is_open )
		{
		...
		if ( hovered && input.mouse_clicked( ) )
		{
			...
			{
				overlays.add<multicombo_overlay>( id, button_rect, width, items, items_count, selected_state.data( ), require_one_selected );
			}
		}
		...
		else
		{
			...
			if ( selected_count == items_count && items_count > 0 )
			{
				display_text = "All";
			}
			else
			{
				if ( local_display_text.empty( ) )
				{
					display_text = std::string_view{ "None" };
				}
				else
				{
					display_text = std::string_view{ local_display_text };
				}
			}
		}
		...
		if ( needs_scroll )
		{
			...
			const auto clip_left = std::max( text_x - 2.0f, win_clip_left );
			...
			if ( scroll_state.scroll_offset > 0.01f )
			{
				const auto gap{ 30.0f };
		...
		for ( int i = 0; i < items_count; ++i )
		{
			selected_items[ i ] = selected_state[ i ] != 0u;
		}
	...
	bool color_picker( std::string_view label, zdraw::rgba& color, float width, bool show_alpha )
	{
		...
		auto changed{ false };
		auto& picker_modes = get_color_picker_modes( );
		auto& picker_base_colors = get_color_picker_base_colors( );
		auto& picker_rainbow_speeds = get_color_picker_rainbow_speeds( );
		auto& picker_rainbow_phase_offsets = get_color_picker_rainbow_phase_offsets( );
		auto& picker_targets = get_color_picker_targets( );
		auto mode_it = picker_modes.try_emplace( id, static_cast< int >( color_picker_mode::static_mode ) ).first;
		mode_it->second = static_cast< int >( clamp_color_picker_mode( mode_it->second ) );
		auto base_it = picker_base_colors.try_emplace( id, color ).first;
		auto speed_it = picker_rainbow_speeds.try_emplace( id, 1.0f ).first;
		auto phase_it = picker_rainbow_phase_offsets.try_emplace( id, 0.0f ).first;
		picker_targets[ id ] = &color;
		speed_it->second = std::clamp( speed_it->second, k_color_picker_rgb_speed_min, k_color_picker_rgb_speed_max );
		const auto mode = clamp_color_picker_mode( mode_it->second );

		if ( !is_open && mode == color_picker_mode::static_mode )
		{
			base_it->second = color;
		}
		...
		if ( mode == color_picker_mode::rainbow )
		{
			color = evaluate_color_picker_preview( mode, base_it->second, speed_it->second, phase_it->second );
		}
		else
		{
			color = base_it->second;
		}
		...
		const auto preview_col = evaluate_color_picker_preview( mode, base_it->second, speed_it->second, phase_it->second );

		zdraw::rect_filled( swatch_rect.x, swatch_rect.y, swatch_rect.w, swatch_rect.h, style.color_picker_bg );

		constexpr auto pad{ 2.0f };
		zdraw::rect_filled( swatch_rect.x + pad, swatch_rect.y + pad, swatch_rect.w - pad * 2.0f, swatch_rect.h - pad * 2.0f, preview_col );
	...
	bool text_input( std::string_view label, std::string& text, std::size_t max_length, std::string_view hint )
	{
		...
		auto async_click_outside{ false };
		if ( is_active )
		{
			constexpr auto async_mouse_slot{ 9000 };
			const auto lmb_down_async = ( ::GetAsyncKeyState( VK_LBUTTON ) & 0x8000 ) != 0;
			const auto lmb_was_down_async = state.key_was_down[ async_mouse_slot ];
			state.key_was_down[ async_mouse_slot ] = lmb_down_async;
			async_click_outside = lmb_down_async && !lmb_was_down_async && !hovered;
		}

		if ( is_active && ( ( input.mouse_clicked( ) && !hovered ) || async_click_outside ) )
		{
		...
		if ( is_active )
		{
			...
			const auto alt_held = ( input_state.key_down_map.count( VK_MENU ) && input_state.key_down_map.at( VK_MENU ) ) || ( input_state.key_down_map.count( VK_LMENU ) && input_state.key_down_map.at( VK_LMENU ) ) || ( input_state.key_down_map.count( VK_RMENU ) && input_state.key_down_map.at( VK_RMENU ) );
			...
			auto process_key = [ &state, &input_state, dt ]( int vk ) -> bool
				{
					...
					else
					{
						...
						else if ( state.key_repeat_timers[ vk ] >= initial_delay )
						{
							const auto excess = state.key_repeat_timers[ vk ] - initial_delay;
							const auto repeats = static_cast< int >( excess / repeat_rate );
							const auto prev_repeats = static_cast< int >( ( excess - dt ) / repeat_rate );
					...
					// Fallback when overlay input queue misses key events (game window focused).
					constexpr auto async_slot_offset{ 4000 };
					const auto async_slot = vk + async_slot_offset;
					const bool async_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
					const auto async_was_down = state.key_was_down[ async_slot ];
					state.key_was_down[ async_slot ] = async_down;

					if ( !async_down )
					{
						state.key_repeat_timers[ async_slot ] = 0.0f;
					}
					else
					{
						state.key_repeat_timers[ async_slot ] += dt;
						if ( !async_was_down )
						{
							triggered = true;
						}
						else if ( state.key_repeat_timers[ async_slot ] >= initial_delay )
						{
							const auto excess = state.key_repeat_timers[ async_slot ] - initial_delay;
							const auto repeats = static_cast< int >( excess / repeat_rate );
							const auto prev_repeats = static_cast< int >( ( excess - dt ) / repeat_rate );
			...
			const auto push_typed_char = [ & ]( char ch )
				{
					...
					const auto has_local_selection = state.selection_start != state.selection_end;
					if ( has_local_selection )
					{
						const auto local_sel_min = std::min( state.selection_start, state.selection_end );
						const auto local_sel_max = std::max( state.selection_start, state.selection_end );
						text.erase( local_sel_min, local_sel_max - local_sel_min );
						state.cursor_pos = local_sel_min;
						state.selection_start = local_sel_min;
						state.selection_end = local_sel_min;
					}
			...
			bool got_wm_char_input{ false };
			for ( const auto wch : input_state.chars( ) )
			{
				if ( wch >= 0 && wch <= 255 )
				{
					push_typed_char( static_cast< char >( wch ) );
					got_wm_char_input = true;
				}
			}

			// Fallback path for overlays where WM_CHAR is not delivered while the game window has focus.
			if ( !got_wm_char_input && !ctrl_held && !alt_held )
			{
				bool got_keypress_input{ false };
				for ( const auto pressed_key : input_state.key_presses( ) )
				{
					const auto ch = vk_to_char( pressed_key, shift_held );
					if ( ch != '\0' )
					{
						push_typed_char( ch );
						got_keypress_input = true;
					}
				}

				// If key_presses is empty (common when overlay window is not the foreground target),
				// sample keyboard edges directly like slider typing does.
				if ( !got_keypress_input )
				{
					const auto async_shift_held =
						( ::GetAsyncKeyState( VK_SHIFT ) & 0x8000 ) != 0 ||
						( ::GetAsyncKeyState( VK_LSHIFT ) & 0x8000 ) != 0 ||
						( ::GetAsyncKeyState( VK_RSHIFT ) & 0x8000 ) != 0;

					const auto key_pressed_or_edge_async = [ &state ]( int vk, int slot_offset = 1000 )
						{
							const auto slot = vk + slot_offset;
							const bool is_down = ( ::GetAsyncKeyState( vk ) & 0x8000 ) != 0;
							const auto was_down = state.key_was_down[ slot ];
							state.key_was_down[ slot ] = is_down;
							return is_down && !was_down;
						};

					const auto emit_async_vk = [ & ]( int vk )
						{
							if ( key_pressed_or_edge_async( vk ) )
							{
								push_typed_char( vk_to_char( vk, async_shift_held ) );
							}
						};

					for ( auto vk = 'A'; vk <= 'Z'; ++vk )
					{
						emit_async_vk( vk );
					}
					...
					for ( auto vk = VK_NUMPAD0; vk <= VK_NUMPAD9; ++vk )
					{
						if ( key_pressed_or_edge_async( vk ) )
						{
							push_typed_char( static_cast< char >( '0' + ( vk - VK_NUMPAD0 ) ) );
						}
					}

					emit_async_vk( VK_SPACE );
					emit_async_vk( VK_OEM_1 );
					emit_async_vk( VK_OEM_PLUS );
					emit_async_vk( VK_OEM_COMMA );
					emit_async_vk( VK_OEM_MINUS );
					emit_async_vk( VK_OEM_PERIOD );
					emit_async_vk( VK_OEM_2 );
					emit_async_vk( VK_OEM_3 );
					emit_async_vk( VK_OEM_4 );
					emit_async_vk( VK_OEM_5 );
					emit_async_vk( VK_OEM_6 );
					emit_async_vk( VK_OEM_7 );
				}
			}
