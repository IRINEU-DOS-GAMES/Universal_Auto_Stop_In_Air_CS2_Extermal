...
namespace features::combat {
	...
	namespace detail {
		...
		static bool ray_capsule_hit_distance(
			...
			float& out_dist )
		{
			...
			if ( capsule_length < 0.001f )
			{
				...
				const auto closest = ray_origin + ray_dir * projection;
				if ( ( closest - capsule_start ).length_sqr( ) > radius * radius )
				{
					return false;
				}
	...
	bool shared::penetration::run( const math::vector3& start, const math::vector3& end, const systems::collector::player& target, const systems::bones::data& bones, result& out ) const
	{
		...
		struct blocker_entry
		{
			const systems::collector::player* player{};
			systems::bones::data bones{};
		};

		std::vector<blocker_entry> blockers{};
		{
			const auto all_players = systems::g_collector.players( );
			blockers.reserve( all_players.size( ) );

			for ( const auto& p : all_players )
			{
				if ( p.pawn == 0 || p.pawn == target.pawn )
				{
					continue;
				}

				if ( p.invulnerable || p.health <= 0 || p.hitboxes.count <= 0 )
				{
					continue;
				}

				const auto p_bones = systems::g_bones.get( p.bone_cache );
				if ( !p_bones.is_valid( ) )
				{
					continue;
				}

				blockers.push_back( { .player = &p, .bones = p_bones } );
			}
		}
		...
		auto check_target = [ & ]( const math::vector3& seg_start, float seg_start_dist, float seg_end_dist ) -> bool
			{
				...
				auto best_total_dist = std::numeric_limits<float>::max( );
				...
				for ( const auto& hb : target.hitboxes )
				{
					...
					float hit_dist{};
					if ( !detail::ray_capsule_hit_distance( seg_start, direction, capsule_start, capsule_end, hb.radius, hit_dist ) )
					{
						continue;
					}
					...
					const auto total_hit_dist = seg_start_dist + hit_dist;
					if ( !found_hit || total_hit_dist < best_total_dist )
					{
						best_total_dist = total_hit_dist;
						best_hit.ray_dist = hit_dist;
						best_hit.damage = damage;
						best_hit.hitbox = hb.index;
						found_hit = true;
					}
				}
				...
				// Do not claim target damage when another player capsule is hit first.
				constexpr auto blocker_eps{ 0.001f };
				for ( const auto& blocker : blockers )
				{
					for ( const auto& hb : blocker.player->hitboxes )
					{
						if ( hb.index < 0 || hb.bone < 0 )
						{
							continue;
						...
						const auto& bone = blocker.bones.bones[ hb.bone ];
						const auto center_local = ( hb.mins + hb.maxs ) * 0.5f;
						const auto center_world = bone.position + math::helpers::rotate_by_quat( bone.rotation, center_local );
						...
						float blocker_dist{};
						if ( !detail::ray_capsule_hit_distance( seg_start, direction, capsule_start, capsule_end, hb.radius, blocker_dist ) )
						{
							continue;
						}

						if ( blocker_dist < 0.0f )
						{
							continue;
						}

						if ( ( seg_start_dist + blocker_dist ) > seg_end_dist )
						{
							continue;
						}

						if ( blocker_dist + blocker_eps < best_hit.ray_dist )
						{
							return false;
						}
					}
				...
				out.damage = best_hit.damage;
				out.hitbox = best_hit.hitbox;
